 ...
 * runUMRNS.java
 * Package:  rtgrid.exp
 * @author Xuan, 2006-11-12 
 */
public class runUMRNS {
	/**
	 * @param args
	 */
...
//		read configuration
		...
		if (run_sim) {
			...
			try {
				...
				out_rej = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-rej.rst");
				p_rej = new PrintStream( out_rej );
				out_que = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-que.rst");
				p_que = new PrintStream( out_que );
				out_rtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-rtime.rst");
				p_rtime = new PrintStream( out_rtime );
				out_wtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-wtime.rst");
				p_wtime = new PrintStream( out_wtime );
				out_adworkload = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-adworkload.rst");
				p_adworkload = new PrintStream( out_adworkload );
				out_miss = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-miss.rst");
				p_miss = new PrintStream( out_miss );
				out_nmin = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-nmin.rst");
				p_nmin = new PrintStream( out_nmin );
				out_details = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-UMR-rej-mn-details.rst");
...
//							switch (i) {
//							case 0 :
//								sch=new SchFIFOMN_UMR(cl);
//								break;
//							case 1 :
//								sch=new SchFIFOAN_UMR(cl);
//								break;
//							case 2:
//								sch=new SchEDFMN_UMR(cl);								
//								break;	
//							case 3:
//								sch=new SchEDFAN_UMR(cl);								
//								break;	
//							case 4:
//								sch=new SchMWF_UMR(cl);
//							}
