...
//		read configuration
		...
		if (run_sim) {
			...
			try {
				...
				out_rej = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-rej.rst");
				p_rej = new PrintStream( out_rej );
				out_que = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-que.rst");
				p_que = new PrintStream( out_que );
				out_rtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-rtime.rst");
				p_rtime = new PrintStream( out_rtime );
				out_wtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-wtime.rst");
				p_wtime = new PrintStream( out_wtime );
				out_adworkload = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-adworkload.rst");
				p_adworkload = new PrintStream( out_adworkload );
				out_miss = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-miss.rst");
				p_miss = new PrintStream( out_miss );
				out_nmin = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-nmin.rst");
				p_nmin = new PrintStream( out_nmin );
				out_details = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-iit-rej-mn-details.rst");
