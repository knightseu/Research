...
/**
 * run experiments for all single algorithms.
 * @author Xuan
 *
 */
public class runSingleNS  {
...
//		read configuration
		...
		if (run_sim) {
			...
			try {
				...
				out_rej = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-rej.rst");
				p_rej = new PrintStream( out_rej );
				out_que = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-que.rst");
				p_que = new PrintStream( out_que );
				out_rtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-rtime.rst");
				p_rtime = new PrintStream( out_rtime );
				out_wtime = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-wtime.rst");
				p_wtime = new PrintStream( out_wtime );
				out_adworkload = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-adworkload.rst");
				p_adworkload = new PrintStream( out_adworkload );
				out_miss = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-miss.rst");
				p_miss = new PrintStream( out_miss );
				out_nmin = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-nmin.rst");
				p_nmin = new PrintStream( out_nmin );
				out_details = new FileOutputStream(COMMON.PARA_OUTPUT_DIR+seperator+"exp-single-rej-mn-details.rst");
				...
				for (int j=1;j<11;j+=1) {
					...
					for (int i=0;i<5;i++){
						...
						for (int k=0;k<COMMON.PARA_EXP_ITERATION;k++) {
							...
							switch (i) {
							case 0 :
								//sch=new SchFIFOMN(cl);
								sch=new SchMN(cl);
								break;
							case 1 :
								sch=new SchFIFOAN(cl);
								break;
							case 2:
								sch=new SchMNEDF(cl);								
								break;	
